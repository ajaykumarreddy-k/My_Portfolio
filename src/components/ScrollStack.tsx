...
const ScrollStack = ({
  ...
  // High-performance animation cache to completely eliminate Layout Thrashing (no getBoundingClientRect on scroll)
  ...
  // Caches card scroll-trigger metrics once on mount and on resize, keeping scroll handlers incredibly lightweight
  ...
  const updateCardTransforms = useCallback(() => {
    ...
    const containerHeight = containerHeightRef.current || (useWindowScroll ? window.innerHeight : scrollerRef.current?.clientHeight || 0);
  ...
  useLayoutEffect(() => {
    ...
    // Cache offsets on initial paint and bind resize handler to completely avoid scroll-based layout recalculations
    cacheOffsets();
    setupLenis();
    ...
    // Timeout buffer for secondary DOM updates
